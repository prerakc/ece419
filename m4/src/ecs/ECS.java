...
public class ECS {
    ...
    public static final String ZOOKEEPER_SERVER_SHELL = System.getProperty("user.dir")
            + "/zookeeper-3.4.11/bin/zkServer.sh";
    public static final String ZOOKEEPER_CONF = System.getProperty("user.dir")
            + "/zookeeper-3.4.11/conf/zoo_sample.cfg";
    ...
    public ECS(String address, int port, boolean ecsMaster) {
        ...
        if (ecsMaster) {
            try {
                ECS.zkMng.getZNodeChildrenCallback("/workers", ECS.serverListWatchCallback(),
                        ECS.getChildrenChangeCallback());
    ...
    public static void addECSNode(String address, int port) {
        ...
        if (ECS.hr.getHashRing().size() == 1) {
            ...
            ECS.hr.getFirstValue().assignHashRange(HashRing.incrementHexString(newNode.getIpPortHash()),
                    newNode.getIpPortHash());
            // ECS.hr.getFirstValue().assignHashRange((new
            // BigInteger(newNode.getIpPortHash(), 16).add(BigInteger.ONE)).toString(16),
            // newNode.getIpPortHash());
        } else {
            // TODO CORNER CASE OF LESS THAN 3 NODES, UPDATE THE STATUS OF NODES DURRING
            // CHANGE
    ...
    public static void removeECSNode(String address, int port) {
        ...
        if (ECS.hr.getHashRing().size() == 1) {
            ...
            // ECS.hr.getFirstValue().assignHashRange(removeNode.getIpPortHash(),
            // ECS.hr.getFirstValue().getNodeHashRange()[1]);
            ECSNode onlyNode = ECS.hr.getFirstValue();
            ECS.hr.getFirstValue().assignHashRange(HashRing.incrementHexString(onlyNode.getIpPortHash()),
                    onlyNode.getIpPortHash());
            // ECS.hr.getFirstValue().assignHashRange((new
            // BigInteger(onlyNode.getIpPortHash(), 16).add(BigInteger.ONE).toString(16)),
            // onlyNode.getIpPortHash());
        ...
        return new Watcher() {
            public void process(WatchedEvent e) {
                if (e.getType() == EventType.NodeDataChanged) {
                    ...
                    try {
                        KVServer.updateStatusZK(
                                ECS.zkMng.getZNodeDataCallback(e.getPath(), ECS.serverStatusWatchCallback()));
                    } catch (InterruptedException | IOException | KeeperException ex) {
                        logger.error("Unexpected exception", ex);
                    }
                }
        ...
        return new Watcher() {
            public void process(WatchedEvent e) {
                if (e.getType() == EventType.NodeDataChanged) {
                    logger.info(String.format("Server status change on %s", e.getPath()));
                    try {
                        KVServer.updateMetadataZK(
                                ECS.zkMng.getZNodeDataCallback(e.getPath(), ECS.workerDataWatchCallback()));
                    } catch (InterruptedException | IOException | KeeperException ex) {
                        logger.error("Unexpected exception", ex);
                    }
                }
    ...
    public static synchronized void updateKeyRange(String path, List<String> workerNames) {
        ...
        // ECS.zkMng.update("/server_status/"+critNode,
        // Integer.toString(StatusType.SERVER_STOPPED.ordinal()).getBytes());
        ...
        if (workerNames.size() > kvCache.size()) {
            ...
            for (String name : kvCache) {
                try {
                    ECS.zkMng.update("/server_status/" + name,
                            Integer.toString(StatusType.SERVER_WRITE_LOCK.ordinal()).getBytes());
                } catch (KeeperException | InterruptedException e) {
                    logger.error("Unable to update server status", e);
                }
            }
        ...
        for (String name : workerNames) {
            try {
                ECS.zkMng.update("/server_status/" + name,
                        Integer.toString(StatusType.SERVER_IDLE.ordinal()).getBytes());
            } catch (KeeperException | InterruptedException e) {
                logger.error("Unable to update server status", e);
            }
        }
    ...
    public void startZookeeper() {
        try {
            ProcessBuilder zookeeperProcessBuilder = new ProcessBuilder(ZOOKEEPER_SERVER_SHELL, "start", ZOOKEEPER_CONF)
                    .inheritIO();

...
public class HashRing {
    ...
    // TODO: may need to swtich to different ordered map structure (conncurrent
    // skiplist map)
    ...
    public Collection<ECSNode> getServers() {
    ...
    public boolean isReplicaOfNode(ECSNode coordinator, ECSNode node) {
        ...
        if (firstSuccessor == null)
            return false;
        if (firstSuccessor.getIpPortHash().equals(node.getIpPortHash()))
            return true;
        ...
        if (secondSuccessor == null)
            return false;
        if (secondSuccessor.getIpPortHash().equals(node.getIpPortHash()))
            return true;
    ...
    public static boolean keyInRange(String key, String[] hashRange) {
        ...
        if (hashRange[0].compareTo(hashRange[1]) < 0) {// if hash range does not wrap around
            return (key.compareTo(hashRange[0]) > 0) && (key.compareTo(hashRange[1]) <= 0); // lower hash is bigger than
                                                                                            // key and higher hash is
                                                                                            // lower
        }
